...
| 2026-05 | view-bonus: ボーナステーブル未適用・権限不足時は 200 + 琥珀案内（赤の汎用失敗にしない） | `b06ece1` |
| 2026-05 | ホーム: 今日の進捗100%で小ゴール達成カード・次の一手ラベル、進捗の storage 再同期 | `e851f46` |
| 2026-05 | 状態チェック: 小ゴール一手は「完了」ボタンに変更（完了→+pt→任意メモ、進捗100%で未完了に残さない） | `bd0b457` |
| 2026-05 | AGARIゴールを大中小+1歩(固定/変動)+今日の1歩に再設計（診断は参考の1歩、完了+3pt、任意メモ） | `8daa53a` |
| 2026-05 | 状態チェック: 今日の1歩を「候補選択→いま取り組む」の1件に固定（POST応答でID確定、`これを選ぶ`） | `c285385` |
